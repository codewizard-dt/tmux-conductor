...
- [TASK-058 — Security hardening + structured logs + heartbeat](TASK-058-security-hardening-logs-heartbeat.md) — security checklist pass (token hashing, single-use pairing, ownership-404, revoke-closes-WS, cookie flags, relay header safelist, caps) + logs + device last-seen heartbeat · todo
- [TASK-059 — README install one-liner + docs updates](TASK-059-readme-docs-updates.md) — install one-liner + CLAUDE.md/scripts/README/.env.example updates for host-server / app/ split + invite-code onboarding · todo
