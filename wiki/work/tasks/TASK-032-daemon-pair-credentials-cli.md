...
- [ ] Use Serena `list_dir` on `daemon/` to confirm the daemon service structure (likely `daemon/index.ts`, maybe existing `daemon/` submodules). Note how the daemon boots and what utilities exist.
- [ ] Use Serena `find_file` for `bin/conductor` to note the existing subcommand dispatch pattern (the `install` and `daemon` subcommands serve as the template).
- [ ] Use Serena `search_for_pattern` for `CONDUCTOR_HOME` in `bin/conductor` and `daemon/` — confirm how the home dir is resolved (likely `${CONDUCTOR_HOME:-$HOME/.local/share/tmux-conductor}`).
- [ ] Check `.gitignore` for any existing `device.json` or `*.json` credential entries.

### 2. Implement `daemon/credentials.ts`  <!-- agent: general-purpose -->

- [ ] Create `daemon/credentials.ts` with:
...
- [ ] All imports from Node builtins only (`node:fs`, `node:path`, `node:child_process` for git root).

### 3. Implement `daemon/pair.ts`  <!-- agent: general-purpose -->

- [ ] Create `daemon/pair.ts` exporting `async function pairDevice(opts: { portalUrl?: string; code?: string }): Promise<void>`.
- [ ] If `opts.portalUrl` is absent: prompt `Enter portal URL [https://...]: ` via readline (open `/dev/tty`); validate it starts with `https://` or `http://localhost` (warn if http on non-localhost).
- [ ] If `opts.code` is absent: prompt `Enter pairing code (XXXX-XXXX): ` via readline.
- [ ] Normalise the code: uppercase, strip dashes and spaces.
- [ ] POST to `${portalUrl}/api/pair/redeem` with `Content-Type: application/json` body `{ code: normalised }`. Use the built-in Node `fetch` (Node ≥22 global — no axios dependency).
- [ ] Handle non-200 responses: parse the body's `error` field and throw a user-friendly `Error` (e.g. `Pairing failed: invalid_or_expired_code — generate a new code from the portal.`).
- [ ] On success: call `writeCredentials({ portalUrl, deviceId: resp.deviceId, token: resp.token })`.
- [ ] Log: `✓ Paired device ${resp.deviceId} to ${portalUrl}. Credentials saved to ${credentialsPath()}.`

### 4. Wire `conductor pair` and `conductor unpair` into `bin/conductor`  <!-- agent: general-purpose -->

- [ ] In `bin/conductor`, add a `pair)` case in the main subcommand dispatch:
...
- [ ] Add both to the `conductor help` output.
- [ ] Ensure `bash -n bin/conductor` passes after changes.

### 5. Typecheck and smoke test  <!-- agent: general-purpose -->

- [ ] Run `npx tsc --noEmit` from the daemon root (or whichever tsconfig covers `daemon/`). Zero type errors.
- [ ] Smoke test `conductor pair --help` / `conductor unpair --help` print their usage lines.
- [ ] Smoke test `conductor unpair` with no `device.json` present — should print "Unpaired" (no error, idempotent).
...
- [ ] Any scratch output goes under `./tmp/pair-smoke/`. Never `/tmp`.

## Acceptance Criteria

- [ ] `daemon/credentials.ts` exports `readCredentials`, `writeCredentials`, `deleteCredentials`, `credentialsPath`; writes atomically with `chmod 600`; appends `device.json` to `.gitignore` idempotently on write.
- [ ] `daemon/pair.ts` exports `pairDevice(opts)`: prompts for portal URL / code when not supplied, POSTs to `/api/pair/redeem`, calls `writeCredentials` on success, throws a user-friendly error on API failure.
- [ ] `conductor pair` subcommand passes `--portal` and `--code` flags through to `daemon/pair.ts`, prompts interactively when flags are absent.
- [ ] `conductor unpair` removes `device.json`; with `--revoke` calls `DELETE /api/devices/:id` before deletion.
- [ ] `bash -n bin/conductor` and `npx tsc --noEmit` both pass cleanly.
