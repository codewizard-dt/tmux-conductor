...
- **`app/api/`** (was `portal/`) — public-facing Fastify + better-auth API on port **8080** in prod / **8090** in local dev (env `API_PORT`), backed by **Postgres** (`DATABASE_URL`). Reaches the host-server via `host.docker.internal:8788` in local Docker dev (`HOST_SERVER_URL`) and via the outbound relay in production. Deployed to DigitalOcean **App Platform** as a Docker service (`app/api/Dockerfile`). Server-side route prefixes: `/api/auth/*` (better-auth), `/api/pair/code` + `/api/pair/redeem`, `/api/devices` (GET) + `/api/devices/:id` (PATCH/DELETE), `/api/invite-codes/validate`, `/api/admin/invite-codes` (GET/POST) + `/api/admin/invite-codes/:id` (DELETE), `/relay/:deviceId` (WS) + `/relay/:deviceId/*` (HTTP catch-all, mux-forwarded to the connected daemon), `/healthz`.
- **`app/frontend/`** (was `frontend/`) — Vite React SPA on port **4321** (env `FRONTEND_PORT`). Deployed to App Platform as a **static site** built with `npm ci && npm run build`. Runtime mode detection: in **direct mode** the SPA talks to the host-server through the Vite dev proxy; in **relay mode** it routes conductor API calls through `/relay/<deviceId>/api`.
...
### Relay data path & onboarding

- **Relay data path:** browser → `app/api` (:8080) → outbound **WSS** → daemon relay connector → host-server (:8788). The public API never reaches the host-server inbound; the host (via its daemon connector) dials *out* to `GET /relay/:deviceId` over WSS and the API mux-forwards browser HTTP calls (`/relay/:deviceId/*`) down that socket to the connected daemon.
- **Invite-code signup gating:** new users must supply a valid invite code to sign up. An admin mints codes via `POST /api/admin/invite-codes`; signup validates the entered code via `POST /api/invite-codes/validate`. Admin gating is **dual-source and must agree**: the server authorizes admins via `BOOTSTRAP_ADMIN_EMAIL` (in `app/api`) and the frontend gates the admin UI via `VITE_ADMIN_EMAIL` — **both must be set to the same email** or the admin screens and the API will disagree. Note: `app/api` runs under `tsx watch`, which does **not** reload on `.env` changes — restart `app/api` manually after editing `BOOTSTRAP_ADMIN_EMAIL` (or any other env var).
- **Pairing (server side only, shipped):** `POST /api/pair/code` (authed; returns an `XXXX-XXXX` code with a 10-minute TTL) and `POST /api/pair/redeem` (a daemon redeems the code → `{ token, deviceId }`).
- **Pairing client (shipped):** the flow is `conductor pair --portal <url> --code XXXX-XXXX` (the `pair)` subcommand in `bin/conductor` — **not** `scripts/conductor.sh`, which is the tmux session orchestrator). It shells into `daemon/pair.ts` (`pairDevice`), which prompts for any missing portal/code, POSTs the code to `POST /api/pair/redeem` → `{ token, deviceId }`, and persists credentials via `daemon/credentials.ts` `writeCredentials()` to `$CONDUCTOR_HOME/device.json` (default `~/.local/share/tmux-conductor/device.json`, written `0600`, atomically via a `.tmp` rename, with `device.json` auto-added to `.gitignore`). `conductor unpair [--revoke]` removes the local credentials (and optionally calls `DELETE /api/devices/:deviceId`). The daemon relay connector lives in `daemon/connector.ts` (`RelayConnector` / `startConnector`): it reads the credentials, derives a `ws(s)://` base from `portalUrl`, and dials `GET /relay/:deviceId` over an outbound WebSocket with a Bearer token, mux-forwarding inbound `/api/*` frames to the host-server and `/daemon/*` frames to the daemon Unix socket (with reconnect backoff). `install.sh` (repo root) bootstraps the install and runs an optional pairing step (`conductor pair <code>`). The only genuinely-pending piece is the live public deployment (TASK-050) — the hosted portal URL isn't live yet, but the install + pairing + relay client code is all present.
- **Vite dev proxy `/api/*` split:** the frontend dev proxy routes the auth/portal prefixes — `/api/auth`, `/api/invite-codes`, `/api/admin/invite-codes`, `/api/devices`, `/api/pair` — to `app/api`, and forwards **everything else** under `/api/*` to the host-server on :8788 (the conductor API).
